...
function create_apple_script(command, sync) {
  ...
  DoScript "${esc(command.toString(), { quotes: 'double', 'minimal': true })}"
end tell`;
...
module.exports = {
  ...
  execute: function(command) {
    ...
    .then( apple_script => new Promise((resolve,reject) => {
      exec(`osascript ${apple_script}`, command.options.execOptions, (err, stdout, stderr) => {
  ...
  executeSync : function(command) {
    ...
    //Execute AppleScript
    try {
      execSync(`osascript ${apple_script}`, command.options.execOptions);
    } catch (err) {
