...
module.exports = {
  ...
  execute : function(command) {
    ...
    .then( jsx => new Promise( resolve => {
      const noui = command.options.noui ? '-noui' : '';
      const cwd = command.options.executablePath ? path.join(command.options.executablePath) : jsx.program_path;
      const options = Object.assign({}, {cwd}, command.options.execOptions);
      exec(`afterfx.exe -r ${noui} ${jsx.script}`, options, (/*err, stdout, stderr*/) => {
        // see * below
  ...
  executeSync : function(command) {
    ...
    try {
      const options = Object.assign({}, {cwd}, command.options.execOptions);
      execSync(`afterfx.exe -r ${noui} ${jsx.script}`, options);
    } catch (err) {
      //TODO *
